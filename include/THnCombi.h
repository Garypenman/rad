...
namespace rad {
 ...
 if constexpr (sizeof...(Args) > 0) {
            ...
            if constexpr (HasMask) {
                ...
                // Robust check: Is the mask a Vector (RVec) or Scalar (bool/int)?
                //if constexpr (is_iterable<MaskT>::value) {
                    // CASE 1: Mask is a Boolean Vector [0, 1, 0...]
                    // We iterate through indices i=0..N. 
                    // If mask[i] is true, we fill the histogram using data at index i.
                    size_t n_mask = mask_ref.size();
                ...
                // } else {
                //     // CASE 2: Mask is Scalar (e.g. global event cut)
                //     if (mask_ref) {
